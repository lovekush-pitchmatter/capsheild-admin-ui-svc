...
const Header: React.FC<HeaderProps> = ({ userImage, userName, userDesignation, onMenuClick }) => {
  ...
  const [selectedFlag, setSelectedFlag] = useState<string>("us");
  ...
  const notifications = [
    { user: 'Alina Petrova', action: 'followed you', time: '4h ago', image: 'https://via.placeholder.com/32' },
    { user: 'Annette Black', action: 'liked your post', time: 'Yesterday', image: 'https://via.placeholder.com/32' },
    { user: 'Ronald Richards', action: 'liked your post', time: 'Tuesday', image: 'https://via.placeholder.com/32' },
    { user: '', action: 'Running low on storage space', time: 'Monday', image: '', isWarning: true },
    { user: 'Eleanor Pena', action: 'commented on your post', time: '4 days ago', image: 'https://via.placeholder.com/32' },
  ];

  const messages = [
    { user: 'Leslie Alexander', message: 'Hi, how are you?', time: '4h ago', image: 'https://via.placeholder.com/32' },
    { user: 'Albert Flores', message: 'Meeting at 3 PM', time: 'Yesterday', image: 'https://via.placeholder.com/32' },
    { user: 'Jacob Jones', message: 'Check the new updates', time: '2 days ago', image: 'https://via.placeholder.com/32' },
    { user: 'Jerome Bell', message: 'Hi, how are you?', time: '4h ago', image: 'https://via.placeholder.com/32' },
    ...
    const fetchLocation = async () => {
      try {
        ...
        if (data && data.country_code) {
          ...
          const exists = flags.find((f) => f.code === countryCode);
          if (exists) {
